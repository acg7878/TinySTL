...
class __hash_map_iterator {
 private:
  ...
  using value_type = std::remove_reference_t<
      decltype(std::declval<__node_value_type>().__get_value())>;
 ...
 public:
  ...
  pointer operator->() const { return std::addressof(__i_->__get_value()); }
...
class __hash_map_const_iterator {
 private:
  ...
  using __node_value_type = typename HashIterator::value_type;
  using value_type = std::remove_reference_t<
      decltype(std::declval<const __node_value_type>().__get_value())>;
...
class unordered_map {
 ...
 private:
  ...
  struct __hash_node_value {
    ...
    __hash_node_value(__hash_node_value&& other)
    ...
    __hash_node_value& operator=(__hash_node_value&& other) {
      const_cast<key_type&>(__data.first) =
          std::move(const_cast<key_type&>(other.__data.first));
  ...
  using __hasher_adapter =
      __unordered_map_hasher<key_type, __hash_node_value, hasher, key_equal>;

  // （3）适配 __hash_table 的键相等性比较（由 __unordered_map_key_equal 实现）
  using __key_equal_adapter =
      __unordered_map_key_equal<key_type, __hash_node_value, key_equal, hasher>;

  // （4）重新绑定分配器：将 value_type 的分配器，绑定为 __hash_node_value 的分配器
  using __node_allocator = typename std::allocator_traits<
      Allocator>::template rebind_alloc<__hash_node_value>;

  // （5）底层哈希表的类型（使用我们自己的 __hash_table 实现）
  using __base_hash_table =
      mystl::__hash_table<__hash_node_value,    // 哈希表存储的节点值类型
 ...
 public:
  ...
  using const_iterator =
      __hash_map_const_iterator<typename __base_hash_table::const_iterator>;
  // 桶内迭代器（遍历单个桶的元素）
  using local_iterator =
      __hash_map_iterator<typename __base_hash_table::local_iterator>;
  using const_local_iterator = __hash_map_const_iterator<
      typename __base_hash_table::const_local_iterator>;
  ...
  // （5）拷贝构造
  unordered_map(const unordered_map& other) : __table_(other.__table_) {
  ...
  std::pair<iterator, bool> emplace(Args&&... args) {
    ...
    auto [base_it, inserted] =
        __table_.__emplace_unique(std::forward<Args>(args)...);
    return {iterator(base_it), inserted};
  }

  // -------------------------- 查找与计数接口（覆盖标准核心接口）--------------------------
  // （1）find()：按键查找（返回迭代器）
  iterator find(const key_type& key) { return iterator(__table_.find(key)); }
  ...
  size_type erase(const key_type& key) { return __table_.__erase_unique(key); }

  // （3）按迭代器范围删除
  iterator erase(iterator first, iterator last) {
    auto base_it =
        __table_.erase(first.__get_iterator(), last.__get_iterator());
    return iterator(base_it);
  }
  ...
  const_iterator begin() const noexcept {
    return const_iterator(__table_.begin());
  }
  ...
  const_iterator cend() const noexcept {
    return const_iterator(__table_.end());
  }
  ...
  local_iterator begin(size_type n) {
    return local_iterator(__table_.begin(n));
  }
  local_iterator end(size_type n) { return local_iterator(__table_.end(n)); }
  const_local_iterator begin(size_type n) const {
    return const_local_iterator(__table_.cbegin(n));
  }
  const_local_iterator end(size_type n) const {
    return const_local_iterator(__table_.cend(n));
  }
  ...
  void max_load_factor(float mlf) { __table_.max_load_factor(mlf); }

  // （3）主动扩容桶数（确保桶数 >= n）
  void rehash(size_type n) { __table_.__rehash_unique(n); }

  // （4）预留元素空间（确保能容纳 n 个元素而不扩容）
  void reserve(size_type n) { __table_.__reserve_unique(n); }
  ...
  key_equal key_eq() const { return __table_.key_eq().key_eq(); }
};
...
class __unordered_map_hasher {
 ...
 public:
  // 构造函数（默认、拷贝）
  __unordered_map_hasher() noexcept(
      std::is_nothrow_default_constructible_v<Hash>)
      : __hash_() {}

  __unordered_map_hasher(const Hash& hf) noexcept(
  ...
  size_t operator()(const Key& key) const { return __hash_(key); }

  // 暴露原始哈希函数（供 unordered_map::hash_function() 使用）
  const Hash& hash_function() const noexcept { return __hash_; }

  // 交换两个适配器
  void swap(__unordered_map_hasher& other) noexcept(
...
class __unordered_map_key_equal {
 ...
 public:
  // 构造函数（默认、拷贝）
  __unordered_map_key_equal() noexcept(
      std::is_nothrow_default_constructible_v<KeyEqual>)
      : __key_eq_() {}

  __unordered_map_key_equal(const KeyEqual& ke) noexcept(
  ...
  const KeyEqual& key_eq() const noexcept { return __key_eq_; }

  // 交换两个适配器
  void swap(__unordered_map_key_equal& other) noexcept(
